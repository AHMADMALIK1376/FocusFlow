...
export default function AcademicCalendarViewPage() {
  ...
  const entriesFor = (calId) => calendar.filter(e => e.calendarId === calId);
  ...
  const handleDeleteCalendar = (calId) => {
    ...
    setCalendars(prev => {
      ...
      if (activeCalendarId === calId)
        setActiveCalendarId(remaining.length > 0 ? remaining[remaining.length - 1].id : null);
      return remaining;
    });
  };

  const deleteEntry = (id) => setCalendar(prev => prev.filter(e => e.id !== id));
  const toggleDone = (id) => setCalendar(prev =>
            ...
            <div className="p-4 space-y-4 min-h-[420px]">
              {entries
                ...
                .map(item => (
                  ...
                  >
                    {/* These are special circular indicator buttons — kept as-is */}
