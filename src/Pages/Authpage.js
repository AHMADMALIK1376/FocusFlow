...
// Both JSON animations imported here
      ...
      <div className="relative z-10 bg-white rounded-[50px] shadow-2xl w-[900px] max-w-[95%] min-h-[620px] overflow-hidden border border-white/20">
        ...
        <div className={`absolute top-0 h-full w-full md:w-1/2 ${syncTransition} ${isLogin ? 'left-0' : 'translate-x-full'}`}>
          <div className="relative w-full h-full flex flex-col bg-white items-center pt-28 px-10">
            ...
            {/* 1. THE LOGO - Now hidden during verification */}
            {!isVerify && (
              <div className="mb-4 z-20 animate-in fade-in zoom-in duration-700">
                  <Logo size="small" />
              </div>
            )}
            ...
            <div className="flex flex-col items-center p-12 pt-20 text-center text-white relative z-10">
              ...
              <div className="mt-4">
                ...
                <p className="text-white/70 text-xs italic mt-2">
                    {isLogin ? "Turn your potential into high-performance reality." : "Join the high-performance community."}
                </p>
                {!isVerify && (
