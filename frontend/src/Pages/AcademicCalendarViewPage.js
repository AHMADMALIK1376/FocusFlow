// src/Pages/AcademicCalendarViewPage.js
...
const smBtn = "magic-btn text-[10px] px-3 py-1.5 rounded-token-sm";

// Sparkline with debounced fetching
const MiniSparkline = ({ entryId, width = 60, height = 20 }) => {
  ...
  useEffect(() => {
    ...
    const fetchTrend = async () => {
      try {
        ...
        console.error(`Failed to fetch trend for ${entryId}:`, error);
        setTrendData([]);
      } finally {
        setIsLoading(false);
      }
    ...
    const timer = setTimeout(fetchTrend, 100);
    return () => clearTimeout(timer);
  }, [entryId]); // eslint-disable-line react-hooks/exhaustive-deps

  if (!trendData || trendData.length < 2 || isLoading) return null;
  ...
  return (
    <svg width={width} height={height} className="inline-block">
      ...
      <line x1="0" y1={height * 0.4} x2={width} y2={height * 0.4} stroke="rgb(var(--focus))" strokeWidth="0.5" strokeDasharray="2,2" opacity="0.4" />
    </svg>
  );
};

// Memoized CalendarTable
const CalendarTable = React.memo(function CalendarTable({
  ...
  const getAttendanceColors = (percentage) => {
    if (!percentage && percentage !== 0) return { bg: 'bg-surface-2', text: 'text-muted', bar: 'bg-[rgb(var(--ink)/0.2)]', border: 'border-[rgb(var(--ink)/0.08)]' };
    if (percentage >= 80) return { bg: 'bg-success/10', text: 'text-success', bar: 'bg-gradient-to-r from-success to-success', border: 'border-success/30' };
    if (percentage >= 60) return { bg: 'bg-warn/10', text: 'text-warn', bar: 'bg-gradient-to-r from-warn to-warn', border: 'border-warn/30' };
    return { bg: 'bg-focus/10', text: 'text-focus', bar: 'bg-gradient-to-r from-focus to-focus', border: 'border-focus/30' };
  };
  ...
  return (
    <div className={`w-full overflow-x-auto rounded-token-xl border bg-surface mb-10 transition-all
      ${isActive ? "shadow-neu border-[rgb(var(--ink)/0.08)]"
        : "shadow-neu-sm border-[rgb(var(--ink)/0.05)] opacity-80"}`}
    >
      {/* Title Bar */}
      <div className="px-6 pt-4 pb-3 border-b border-[rgb(var(--ink)/0.08)] flex items-center justify-between gap-4">
        <div className="flex items-center gap-1.5">
          <button onClick={onPrevWeek} disabled={currentWeekOffset <= minWeekOffset}
            className={`w-8 h-8 rounded-token-sm bg-surface-2 shadow-neu-sm flex items-center justify-center text-brand font-bold transition-all
        ...
        <div className="flex-1 flex flex-col items-center min-w-0">
          {editingCalId === cal.id ? (
            <input autoFocus value={editingTitle} onChange={(e) => setEditingTitle(e.target.value)}
              ...
              className="text-center bg-surface-2 shadow-neu-inset rounded-token-sm px-4 py-2 font-black text-brand uppercase tracking-widest text-sm outline-none max-w-[300px]" />
          ) : (
            <div className="group cursor-pointer flex items-center gap-2" onClick={onStartEdit} title="Click to edit">
              <span className="text-base">📋</span>
              <h3 className="text-sm font-black text-brand uppercase tracking-[0.1em] truncate group-hover:text-brand-deep">{cal.title}</h3>
              <span className="opacity-0 group-hover:opacity-100 text-[10px] text-muted">✎</span>
              {isActive && <span className="text-[8px] bg-brand/10 text-brand px-2 py-0.5 rounded-full font-black tracking-widest">ACTIVE</span>}
            </div>
          )}
          <p className="text-[10px] text-muted font-medium mt-0.5">{getWeekLabel ? getWeekLabel() : ''}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {editingCalId === cal.id ? (
            <>
              <button onClick={onSaveEdit} className={`${smBtn} text-success`}>✓ Save</button>
              <button onClick={onCancelEdit} className={`${smBtn} text-muted`}>✕ Cancel</button>
            </>
          ) : (
            <>
              {!isActive && <button onClick={onMakeActive} className={smBtn}>↑ Set Active</button>}
              <button onClick={onStartEdit} className={`${smBtn} text-muted`}>✎ Edit</button>
              <button onClick={onDelete} className={`${smBtn} text-focus`}>🗑 Delete</button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="min-w-[1000px] grid grid-cols-5">
        {displayDays.map((day, index) => (
          <div key={day} className={`flex flex-col ${index !== 4 ? "border-r border-[rgb(var(--ink)/0.08)]" : ""}`}>
            <div className={`p-6 text-center border-b border-[rgb(var(--ink)/0.08)] ${isToday(day) ? 'bg-brand/5' : ''}`}>
              <div className="font-black text-ink uppercase tracking-[0.2em] text-sm">{day}{isToday(day) && <span className="ml-1 text-[10px] text-brand">●</span>}</div>
              <div className="text-[10px] font-bold text-brand mt-1 opacity-70">{getDayDate(day, referenceDate)}</div>
            </div>
            <div className="p-4 space-y-4 min-h-[420px]">
              {entries.filter(e => e.days?.includes(day))
                ...
                .map(item => {
                  ...
                  const colors = getAttendanceColors(percentage);
                  ...
                  const cardCls = isPresent
                    ? "bg-success/10 shadow-neu-inset border border-success/20"
                    : isAbsent
                      ? "bg-focus/10 shadow-neu-inset border border-focus/20"
                      : "bg-surface-2 shadow-neu-sm hover:shadow-neu-inset";

                  return (
                    <div key={item.id} className={`group relative p-4 rounded-token-md transition-all duration-300 ${cardCls}`}>
                      {isPending && (
                        <button
                          onClick={() => onEditEntry && onEditEntry(item, day)}
                          className="absolute -top-2 -right-2 bg-info text-on-brand w-5 h-5 rounded-full text-[8px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all shadow-neu-sm z-10 hover:scale-110"
                      ...
                      {showToggle && (
                        <>
                          <button onClick={() => deleteEntry(item.id)}
                            className="absolute -top-2 right-6 bg-focus text-on-brand w-5 h-5 rounded-full text-[8px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all shadow-neu-sm z-10 hover:scale-110">✕</button>
                          <button onClick={() => toggleDone(item.id)}
                            className={`absolute -top-2 -left-2 w-6 h-6 rounded-full text-[10px] flex items-center justify-center transition-all shadow-neu-sm z-10 hover:scale-110
                              ${isPresent ? "bg-success text-on-brand opacity-100" : "bg-surface border-2 border-success text-success opacity-0 group-hover:opacity-100"}`}>✓</button>
                        </>
                      )}

                      {isPresent && <div className="absolute top-2 right-2 bg-success text-on-brand text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider shadow-neu-sm">Attended</div>}
                      {isAbsent && <div className="absolute top-2 right-2 bg-focus text-on-brand text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider shadow-neu-sm">Absent</div>}

                      <div className="flex items-center justify-between mb-2">
                        <h4 className={`font-black text-[11px] leading-tight uppercase transition-all flex-1 min-w-0 ${isPresent ? "text-success" : isAbsent ? "text-focus" : "text-ink"}`}>{item.subject}</h4>
                        {statusBadge && <span className="text-[10px] ml-1 flex-shrink-0">{statusBadge}</span>}
                      </div>

                      <div className="flex flex-col gap-2">
                        <div className={`flex items-center gap-1.5 text-[10px] font-bold uppercase ${isPresent ? "text-success" : isAbsent ? "text-focus" : "text-muted"}`}>
                          <span className={isPresent ? "text-success" : isAbsent ? "text-focus" : "text-brand"}>●</span>
                          {formatTimeStr(displayStartTime)} - {formatTimeStr(displayEndTime)}
                        </div>
                        <div className={`flex items-center gap-2 text-[10px] font-black self-start px-2 py-1 rounded-token-sm border
                          ${isPresent ? "bg-success/10 text-success border-success/20" : isAbsent ? "bg-focus/10 text-focus border-focus/20" : "bg-[rgb(var(--ink)/0.04)] text-ink border-[rgb(var(--ink)/0.08)]"}`}>
                          <div className="flex flex-col items-center justify-center translate-y-[1px]">
                            <div className={`w-1.5 h-1.5 rounded-full ${isPresent ? "bg-success" : isAbsent ? "bg-focus" : "bg-focus"}`}></div>
                            <div className="w-[1.5px] h-2 bg-[rgb(var(--ink)/0.2)]"></div>
                          </div>
                      ...
                      {attendance && (
                        <div className="mt-3 pt-2 border-t border-[rgb(var(--ink)/0.08)]">
                          ...
                          <div className="flex justify-between mt-1 text-[7px] text-muted font-medium">
                          ...
                          {attendance.isWarning && (
                            <div className="mt-1.5 p-1.5 bg-focus/10 rounded-token-sm border-l-2 border-focus">
                              <p className="text-[7px] text-focus font-bold leading-tight">⚠️ Below 60% - Exam eligibility at risk!</p>
                            </div>
                          )}
                        </div>
                      )}
...
export default function AcademicCalendarViewPage() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center px-4 min-h-screen bg-canvas">
  ...
  if (calendars.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center px-4 min-h-screen bg-canvas">
        ...
        <h2 className="text-2xl font-black text-ink mb-2">No Calendars Yet</h2>
        <p className="text-muted text-sm font-bold mb-8 uppercase tracking-widest">Go create one first</p>
  ...
  return (
    <div className="flex flex-col items-center px-4 pt-10 pb-20 max-w-[1400px] mx-auto min-h-screen bg-canvas">
      ...
      <header className="w-full max-w-[1100px] mb-8 text-center">
        <h1 className="text-4xl md:text-5xl font-black text-brand tracking-[0.2em] uppercase mb-3 drop-shadow-sm">My Calendars</h1>
        <div className="flex items-center justify-center gap-4">
          <div className="h-[2px] w-12 bg-[rgb(var(--ink)/0.15)] rounded-full"></div>
          <p className="text-sm font-bold tracking-[0.3em] text-muted uppercase">Spring <span className="text-brand">( 2026 )</span></p>
          <div className="h-[2px] w-12 bg-[rgb(var(--ink)/0.15)] rounded-full"></div>
        </div>
      </header>
