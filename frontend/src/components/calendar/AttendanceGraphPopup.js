...
const HorizontalBarChart = ({ subjects }) => {
  ...
  const getColor = (pct) => {
    if (pct >= 80) return { main: '#10b981', light: '#d1fae5' };
    if (pct >= 60) return { main: '#f59e0b', light: '#fef3c7' };
    ...
    <div className="flex justify-center w-full" style={{ overflow: 'visible', position: 'relative' }}>
      <svg width="100%" height={totalH} viewBox={`0 0 ${totalW} ${totalH}`} preserveAspectRatio="xMidYMid meet"
        ...
        <line x1={paddingLeft + (chartW * 0.6)} y1={5} x2={paddingLeft + (chartW * 0.6)} y2={sortedSubjects.length * (barH + gap) + 10}
          stroke="#ef4444" strokeWidth="1.5" strokeDasharray="5,3" opacity="0.5" />
        <text x={paddingLeft + (chartW * 0.6)} y={4} textAnchor="middle" fill="#ef4444" fontSize="9" fontWeight="black">60% ⚠️</text>
        ...
        {[0, 25, 50, 75, 100].map((pct) => (
          <text key={pct} x={paddingLeft + chartW * (pct / 100)} y={sortedSubjects.length * (barH + gap) + 30} 
            textAnchor="middle" fill="#9ca3af" fontSize="8">{pct}%</text>
        ))}
        ...
        <text x={paddingLeft + chartW / 2} y={sortedSubjects.length * (barH + gap) + 45} 
          textAnchor="middle" fill="#9ca3af" fontSize="9" fontWeight="bold">
          ...
          return (
            <g key={subject.entryId || i}
              ...
              <text x={paddingLeft - 8} y={y + barH/2 + 4} textAnchor="end" 
                fill="#9ca3af" fontSize="9" fontWeight="bold">
                {sortedSubjects.length - i}
              </text>
              
              <rect x={paddingLeft} y={y} width={chartW} height={barH} rx="6" fill="#e5e7eb" />
              ...
              <text x={paddingLeft + 12} y={y + barH/2 + 4} textAnchor="start" 
                fill="white" fontSize="11" fontWeight="black" opacity="0.95">
      ...
      {hoveredIndex !== null && sortedSubjects[hoveredIndex] && (() => {
        ...
        return (
          <div 
            ...
            <div className="bg-gray-900 text-white rounded-xl px-4 py-3 shadow-2xl min-w-[180px]">
              ...
              <div className="flex gap-3 mt-1.5 text-[10px]">
                <span className="text-green-400">✅ {subject.attendedSessions || 0} Present</span>
                <span className="text-red-400">❌ {subject.absentSessions || 0} Absent</span>
              </div>
              <p className="text-[9px] text-gray-400 mt-1">📅 {taken} taken / {subject.totalSessions || 0} total</p>
            </div>
            <div className="absolute -left-2 top-1/2 -translate-y-1/2 w-3 h-3 bg-gray-900 rotate-45"></div>
          </div>
        );
      })()}
    </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ========== BACK CARD (White, underneath) ========== */}
        <div 
          className="absolute inset-0 bg-white rounded-[22px] shadow-[0_10px_40px_rgba(0,0,0,0.1)]"
          style={{ zIndex: 1 }}
        />
        
        {/* ========== FRONT CARD (Main content) ========== */}
        <div 
          className={`relative bg-[#f0f2f5] rounded-[22px] p-6 z-10 max-h-[85vh] overflow-y-auto
        ...
        >
          ...
          >
            ...
            <div className="absolute top-0 right-0 transition-all duration-700 ease-out"
              style={{
                ...
                background: isHoveringCorner ? '#7C3AED' : 'transparent',
            ...
            <button
              ...
              style={{
                background: isHoveringCorner ? '#ef4444' : 'transparent',
                width: '20px', height: '20px', borderRadius: '50%',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                boxShadow: isHoveringCorner ? '0 2px 8px rgba(239,68,68,0.5)' : 'none',
          ...
          <div className="text-center mb-4">
            <h2 className="text-xl font-black text-gray-800">Attendance Dashboard</h2>
          </div>
            ...
            <>
              <div className="flex justify-center gap-8 mb-4">
                <div className="text-center">
                  <p className="text-2xl font-black text-focusPurple">{attendanceStats.overallPercentage?.toFixed(0)}%</p>
                  <p className="text-[9px] text-gray-400 uppercase tracking-wider">Overall</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-black text-green-600">{attendanceStats.totalEarned || 0}</p>
                  <p className="text-[9px] text-gray-400 uppercase tracking-wider">Points</p>
                </div>
                <div className="text-center">
                  <p className={`text-2xl font-black ${attendanceStats.subjectsAtRisk > 0 ? 'text-red-600' : 'text-green-600'}`}>
                    {attendanceStats.subjectsAtRisk || 0}
                  </p>
                  <p className="text-[9px] text-gray-400 uppercase tracking-wider">At Risk</p>
                </div>
              </div>
              ...
              <p className="text-[7px] text-gray-400 text-center mt-2">Hover over bars for details • Red line = 60% exam eligibility threshold</p>
            </>
          ) : (
